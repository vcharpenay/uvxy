# note: duplicated from pykeen/sampling/basic_negative_sampler.py
...
class BasicNegativeSampler(NegativeSampler):
    ...
    def corrupt_batch(self, positive_batch: torch.LongTensor) -> torch.LongTensor:  # noqa: D102
        ...
        arity = positive_batch.shape[-1]

        # clone positive batch for corruption (.repeat_interleave creates a copy)
        negative_batch = positive_batch.view(-1, arity).repeat_interleave(self.num_negs_per_pos, dim=0)
        ...
        return negative_batch.view(*batch_shape, self.num_negs_per_pos, arity)
